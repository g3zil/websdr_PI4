...
then
    ...
    python3 .py ${DECODE_CAPTURE_DATE} ${BASE_DIR}/12000.wav  > ${BASE_DIR}/PI4_detect.log   # do the processing!
    sed -i 's/^M//g' file ${BASE_DIR}/PI4_detections.csv                  # Have to remove the control M carriage return at end of line
    ...
    awk -F"," -v OFS="," -v tx_call="${TX_CALL}" -v tx_grid="${TX_GRID}" -v band="${BAND}" -v frequency="${FREQUENCY}" \
     -v rx_id="${RX_ID}" -v rx_grid="${RX_GRID}" ' {print $1,tx_call,tx_grid,band,frequency,rx_id,rx_grid,$2,$3,$7,$8,$9,$10,$11,$12,$13,$14,$15,$5,$4}' \
